        ...
        <Col class="clearfix" span="24">
            ...
            <Col span="24">
                <div class="main-block">
                    <Page :total="saleThreadTotalList.length" size="small" show-elevator show-sizer placement="top" @on-change="pageTurn" @on-page-size-change="pageSizeChange"></Page>
                </div>
            </Col>      
        </Col>
       </Row>
        <Row class="clientInfoMain" :style="{right:r+'px'}">
            <sale-thread-user-info @rightChange="rightChange" :selectItem="selectItem"></sale-thread-user-info>
        </Row>
   </div>  
</template>
...
import SaleThreadUserInfo from '@/components/saleThread/saleThreadUserInfo'
...
export default {
  ...
  watch:{
    ...
    saleThreadTotalList:{
      deep:true,
      handler(){
         if(this.saleThreadTotalList.length){
           this.saleThreadList = this.saleThreadTotalList.slice((this.pageNum-1)*this.pageSize,this.pageNum*this.pageSize)
         }
      }
    }
  },
  mounted () {
    ...
    this.$store.dispatch('getSaleThreadList')
  },
  computed:{
    layoutWidth:{ // 容器宽度
      get(){
    ...
    saleThreadTotalList:{ // 所有客户列表信息
      get(){
        return this.$store.state.saleThreadList
      },
      set(){
      }
    },
    saleThreadList:{  // 本页显示信息
      get(){
         if(this.saleThreadTotalList.length){
           return this.saleThreadTotalList.slice((this.pageNum-1)*this.pageSize,this.pageNum*this.pageSize)
         }else{
           return []
         }
      },
